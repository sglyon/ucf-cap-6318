...
# ╔═╡ b5e3fa9b-5a86-4361-905c-e3d4cbc30b5f
using Agents

# ╔═╡ 1dcdec88-66ad-4438-8904-7c72a95ef0ff
using PlutoUI

# ╔═╡ 127c4c46-260c-11ec-05c9-9b0e6396bf7d
md"""
...
# ╔═╡ 290fbb71-b446-4f53-b4f6-42ed0c913374
md"""
...
# ╔═╡ 3c3f189e-beb3-4394-8c00-7f2eb2429120
md"""
## Review: Julia Types

- Also recall how to define a composite type in Julia
```julia
...
# ╔═╡ 96f307e9-6988-4856-8a19-1943969bc3e4
md"""
# Agents.jl
...
# ╔═╡ 24a4f834-a4a6-456b-aa19-d310116308a5
PlutoUI.Resource("https://github.com/sglyon/ucf-cap-6318/raw/Fall21/notebooks/week07/abm_toolkit_comparison.png")

# ╔═╡ 9b03eef1-c8c8-4917-a967-c6765b5558ef
PlutoUI.Resource("https://github.com/sglyon/ucf-cap-6318/raw/Fall21/notebooks/week07/abm_table1.png")

# ╔═╡ eb74e11a-b44c-4e54-9a1a-64a53aa61cce
PlutoUI.Resource("https://github.com/sglyon/ucf-cap-6318/raw/Fall21/notebooks/week07/abm_table2.png")

# ╔═╡ 4be77ed8-1182-4c23-9e2c-dfe6841fb3dc
md"""
...
- The agent must be a subtype of the abstract type `AbstractAgent`, which is defined in Agents.jl
- We also define this as a `mutable` struct so we can update its position attribute
"""

# ╔═╡ 63e3f4bb-498f-473b-8aae-f75148c080ad
NTuple{2,Int}

# ╔═╡ 38362eb0-f825-4c5f-b28f-c21127a327d0
mutable struct SchellingAgent <: AbstractAgent
    id::Int             # The identifier number of the agent
    pos::Tuple{Int64, Int64} # The x, y location of the agent on a 2D grid
...
# ╔═╡ c7c479e7-26af-42f4-b1db-4730264ca01e
agent007 = SchellingAgent(7, (1, 1), false, 42)

# ╔═╡ 86e0afa2-dc3b-4a75-993b-3a8a5f805dd1
md"""
...
# ╔═╡ 659add9b-3928-4f5d-8360-12ad5e64fb32
environment = GridSpace((25, 25); periodic = false)

# ╔═╡ a33545a2-8839-45a0-9f5e-905188f255c0
md"""
## Combining Agents and Environment

- We now need to create an instance of the `AgentBasedModel` (or `ABM` for short) type
- To construct our instance we need to specify our agent type, our environment, and any additional properties
...
# ╔═╡ 34e656d5-283b-443b-9cb5-1434a7d0686f
properties = Dict(:wanted_neighbors => 4)

# ╔═╡ 89c46cab-82e6-4d3f-b7fe-e32cc6324852
schelling = AgentBasedModel(SchellingAgent, environment; properties=properties)

# ╔═╡ 052dc5b2-def3-4509-b982-79e3ddac87d8
md"""
## Rules
...
# ╔═╡ d9b9a2f5-baca-4025-89cb-025effa178a6
...
# ╔═╡ 096920eb-8fec-4fc1-bc87-b64bf06f4f63
md"""
## Add Agents
...
# ╔═╡ 3e9fd523-d47d-4567-bb9a-9194218c01d6
function init_schelling(;num_agents_per_group=250)
	environment = GridSpace((25, 25), periodic=false)
	properties = Dict(:wanted_neighbors => 4)
	model = ABM(
		SchellingAgent, environment; 
...
# ╔═╡ 13723461-fe80-48f4-99cd-4e2f40637252
model = init_schelling()

# ╔═╡ fa8f69b9-9060-4aa0-9163-ad27bd9d0e52
begin
...
# ╔═╡ e5a144d6-f177-4cea-884e-7378de323e39
md"""
## Running the Model
...
# ╔═╡ 79828262-307f-4cef-8b3f-5e8d444f86e9
# advance one step
step!(model, agent_step!)

# ╔═╡ 577c5b9e-6163-4324-8f55-a6e6fbccca0b
# advance three steps
step!(model, agent_step!, 3)

# ╔═╡ 1c3b3d50-9835-44ed-9505-52c1caae9c5f
md"""
...
# ╔═╡ ead7ad4c-6a23-48d6-a04b-2a3e469a5022
let
	model = init_schelling()
	adata = [:pos, :is_happy, :group]
	data, _ = run!(model, agent_step!, 10; adata)
	data
end

# ╔═╡ 373f0f76-a902-4eaf-85a9-60e156da0097
md"""
## Visualizing the Agents

- One of the more instructive (and fun!) parts of agent based modeling is visualizing the data
- To do this visualization we will use the `abm_plot` function
"""

# ╔═╡ c59ea94b-c19c-449c-a9c9-be165b2854d4
md"""
## Animating the Agents

- We can also create a video that animates our agents moving throughout the system
- We do this using the `abm_video` function as follows
"""

# ╔═╡ 52abea2c-cbcc-4e85-b504-558dc1952aeb
let
	model = init_schelling();
	abm_video(
		"schelling.mp4", model, agent_step!;
		ac=groupcolor,  # color
		am=groupmarker, # marker
		as=10,          # size
		framerate=4,    # frames per second
		frames=20,      # number of frames (steps)
		title="Schelling's segregation model"
	)
end

# ╔═╡ c5ea1088-01ba-49fd-b9a6-e788364c755b
PlutoUI.LocalResource("schelling.mp4")

# ╔═╡ 72b6973a-8e9a-43a9-bc7b-30853df4cc24
md"""
## Interactive Application

- Agents.jl also makes it very easy to create an interactive application for our model!
- We can do this using the `abm_data_exploration` function
- This expects the following positional arguments:
  - `model`
  - `agent_step!`
  - `model_step!` (we don't have one, so we pass `dummystep`)
  - `parameter_range`: Dict mapping model parameters to range of values to test
- We also have some keyword arguments
  - `ac`, `am`, `as`: control marker color color, symbol, and size as before
...
# ╔═╡ 38542d74-4b3d-407a-88b7-c72753f38152
let
	model = init_schelling()
	adata = [(:is_happy, sum)]
	alabels = ["n_happy"]
...
# ╔═╡ 8dc2ac80-f827-4a1d-8ba1-54386fa5b341
md"""
### Julia script

- Note that this does not run properly in Pluto, so we'll need to move to a text editor and Julia REPL to see the example work...
- You can find a complete script on GitHub [here](https://github.com/sglyon/ucf-cap-6318/blob/ae28b564978d0016ce6e75e6e4432ba6c8f391d9/scripts/schelling_agents.jl)
"""

# ╔═╡ 13fd4598-1cd8-45bf-a0c8-2cba7d5bf7d0
md"""
# Summary

- Agents.jl is a powerful framework for doing agent based modeling in Julia
- It is fast, expressive (little code), integrated (use all Julia tools), and capable
- We'll be using it to define our agents, environments, and rules for upcoming lectures
- Quiz: which topics interest you?? See [Examples page](https://juliadynamics.github.io/Agents.jl/stable/examples/)
"""
...
git-tree-sha1 = "386f669913c23d8b247552c9f3fb58b8fd5b9dbe"
